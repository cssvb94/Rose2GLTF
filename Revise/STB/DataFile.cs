...
namespace Revise.STB {
    ...
    public class DataFile : FileLoader {
        ...
        public DataRow this[int row] {
            get {
                if (row < 0 || row > rows.Count - 1) throw new ArgumentOutOfRangeException("row", "Row is out of range");
                return rows[row];
            }
        }

        public List<string> RowsData(int row) => rows[row].data;
        ...
        public string GetColumnName(int column) {
            if (column < 0 || column > columns.Count - 1) throw new ArgumentOutOfRangeException("column", "Column is out of range");
