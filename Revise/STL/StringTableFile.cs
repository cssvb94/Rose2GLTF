...
using System.IO;
using Revise.STL.Attributes;

namespace Revise.STL {
    /// <summary>
    ...
    public class StringTableFile : FileLoader {
        ...
        public override void Load(Stream stream) {
            ...
            for (int j = 0; j < rowCount; j++) {
                StringTableRow row = new(languageCount);
                rows.Add(row);
            }
        ...
        public StringTableRow AddRow(string key, int id) {
            StringTableKey tableKey = new();
            tableKey.Key = key;
            tableKey.ID = id;

            StringTableRow tableRow = new(LanguageCount);
